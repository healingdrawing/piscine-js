function addWeek(date) {
    ...
    var daysSinceEpoch = Math.floor((date.getTime() - new Date('0001-01-01').getTime()) / (1000 * 60 * 60 * 24));
    // calculate the day of the 14-day week
    var weekDay = daysSinceEpoch % 14;
    // map the day of the week to a string
    var weekDayString = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday', 'secondMonday', 'secondTuesday', 'secondWednesday', 'secondThursday', 'secondFriday', 'secondSaturday', 'secondSunday'][weekDay];
    // return the string
    return weekDayString;
}
function timeTravel(options) {
    // create a new date object with the original date
    var newDate = new Date(options.date);
