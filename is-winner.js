const isWinner = async (countryName) => {
  ...
  // console.log("country", country) // country { id: 9, name: 'zo54rgcswy', continent: 'Europe' } ... facepalm
  ...
  // check if the country won the FIFA World Cup at least 3 times
  if (results.length < 3) {
    return `${country.name} is not what we are looking for because of the number of times it was champion`
  }

  // get the years and the results
  const years = results.map((result) => result.year)
  const scores = results.map((result) => result.score)

  // return the string
  return `${country.name} won the FIFA World Cup in ${years.join(', ')} winning by ${scores.join(', ')}`
}
