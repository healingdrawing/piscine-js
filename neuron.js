...
const neuron = (arr) => {
  ...
  arr.forEach((el) => {
    const firstType = el.split(": ")[0];
    const firstItem = el.split(": ")[1].split(" - ")[0];
    const secondItem = el.split(" - ")[1].split(": ")[1];

    const questions = firstType.toLowerCase();
    const property = toProp(firstItem);
    const question = questions.substr(0, questions.length - 1);
    const questionItem = firstItem;
    const responses = "responses";
    const response = secondItem;
